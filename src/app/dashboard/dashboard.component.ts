...
// import { ChartOptions, SummaryData } from '../Model/chart-options';
...
export class DashboardComponent implements OnInit {
  ...
  private modelerlist: EmpResult[] = [];
  // private modelvlist: any = [];
  public Activedayviewemps: any = [];
  ...
  count: any = 0;
  Id: any;
  constructor(private RedBenchService: RedBenchService) {
  ...
  getDayview() {
    this.RedBenchService.getdayview().subscribe(
      results => {
        ...
        for (var i = 0; i < this.modeldalist1.length; i++) {
          if (this.modeldalist1[i]) {
            this.Activedayviewemps.push(this.modeldalist1[i]);
            this.count += parseInt(this.Rate);
          }
        }
      },
      error => {
  ...
  getActiveEmps() {
    this.RedBenchService.getActiveemps().subscribe(
      results => {
        ...
        for (var i = 0; i < this.modelelist.length; i++) {
          if (this.modelelist[i].Active == true) {
            this.ActiveEmployees.push(this.modelelist[i]);
            this.count += parseInt(this.Rate);
          }
        }
      },
  ...
  getInActiveEmps() {
    this.RedBenchService.getInActiveemps().subscribe(
      results => {
        ...
        for (var i = 0; i < this.modelerlist.length; i++) {
          if (this.modelerlist[i].Active == false) {
            this.InActiveEmployees.push(this.modelerlist[i]);
            this.count += parseInt(this.Rate);
          }
        }
      },
  ...
  ngOnInit(): void {
    ...
    this.type = this.loggedInUser.Type;
    for (var i = 0; i <= this.mainChartElements; i++) {
      this.mainChartData1.push(this.random(50, 200));
      this.mainChartData2.push(this.random(80, 100));
      this.mainChartData3.push(65);
    }
    if (this.loggedInUser.Type == 'organization') {
